...
export function getMcpConfig() {
  const readConfig = (dir) => {
    const configPath = path.join(dir, ".minicode", "settings.json");
    try {
...
export async function linkMcpAndListTool(targetList, targetMap) {
  ...
  for (const mcpServer of mcpList) {
    try {
      ...
      // 自动推断type：未显式声明type时，有command则默认stdio，有url则默认http
      const resolvedType = type || (command ? "stdio" : url ? "http" : null);

      // 根据type创建对应的transport
      if (resolvedType === "http" || resolvedType === "streamablehttp") {
        transport = new StreamableHTTPClientTransport(url, {
          requestInit: {
            headers,
          },
        });
      } else if (resolvedType === "sse") {
        transport = new SSEClientTransport(new URL(url), {
          requestInit: {
            headers,
          },
        });
      } else if (resolvedType === "stdio") {
